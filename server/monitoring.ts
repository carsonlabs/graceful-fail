...
export class MonitoringRegistry {
  // Counters
  readonly proxyRequests = new Counter("selfheal_proxy_requests_total", "Total proxy requests");
  readonly proxySuccesses = new Counter("selfheal_proxy_successes_total", "Successful proxy pass-throughs (free)");
  readonly proxyFailures = new Counter("selfheal_proxy_failures_total", "Proxy requests that returned errors from target");
  readonly healRequests = new Counter("selfheal_heal_requests_total", "Total heal analysis requests (paid)");
  readonly healSuccesses = new Counter("selfheal_heal_successes_total", "Successful heal analyses");
  readonly healFailures = new Counter("selfheal_heal_failures_total", "Failed heal analyses");
  readonly x402Payments = new Counter("selfheal_x402_payments_total", "Total x402 payments received");
  readonly x402Revenue = new Counter("selfheal_x402_revenue_usdc_total", "Total revenue in USDC atomic units");
  readonly x402Refunds = new Counter("selfheal_x402_refunds_total", "Payments not settled (heal failed)");
  readonly llmTokens = new Counter("selfheal_llm_tokens_total", "Total LLM tokens consumed");

  // Histograms
  readonly proxyLatency = new Histogram("selfheal_proxy_latency_ms", "Proxy request latency in milliseconds");
  ...
  // Gauges
  readonly activeRequests = new Gauge("selfheal_active_requests", "Currently active proxy requests");
  readonly cacheSize = new Gauge("selfheal_cache_size", "Number of entries in response cache");
  readonly cacheHitRate = new Gauge("selfheal_cache_hit_rate", "Cache hit rate (0-1)");
  ...
  private async checkAlerts(): Promise<void> {
    ...
    if (
      this.baselineRpm > 0 &&
      this.lastMinuteRequests > this.baselineRpm * this.alertConfig.trafficSpikeMultiplier
    ) {
      alerts.push({
        severity: "warning",
        type: "traffic_spike",
